...
TEST_ADD(TestCrashReporterStackOverflow);
...
TEST_ADD(TestCommonInlinePrintf);
...
TEST_ADD(TestJSONConfigStateReader);
TEST_ADD(TestJSONConfigStateReader_ReadAndWrite);
TEST_ADD(TestJSONConfigStateReader_Benchmark);
